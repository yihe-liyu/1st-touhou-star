...
- ~~书签数据化~~ → ✅ 已完成（2026-08：BookmarkExtractor + 运行时收集缓存 + 人工编辑）
- ~~命中框~~ → ✅ 已完成（2026-08）
- 形态走廊（多时刻叠影）—— 后续
- **数据化编辑器（设计已定，待实现）—— 见下方专节**

### 🎛️ 数据化编辑器设计（2026-08 讨论定稿，待实现）

> 目标：工作台从"预览器"变"编辑器"——编排节奏/敌人/弹幕用数据描述，改参即重跑。
> 设计经过 5 轮讨论（粒度/交互/重叠/自定义参数/代码逃逸口）与 YiHe 对齐。
...
2. **个体层（后置 S5+）**：敌人行为表（move_to/ring/aim/fan/die 等标准件）
...
**落地路线（待实现）**：
- S1 定义 StageTimeline（波次表）+ WaveSpawner 解释器（复用现有敌人模板）→ 数据驱动跑出红妖精波
- S2 工作台编排表格（波次列表）
- S3 详情面板（schema/反射动态表单）+ [✔ 应用]重跑
- S4 [💾 保存] .tres + 增删波次
- S5+ 个体行为表（敌人内部）

**试验田**：新建数据驱动关卡样板（不迁移现有 stage01，协程版保留）

**实现后的清理标记（YiHe 2026-08 预判）**：
- [ ] 书签"运行时收集 + 缓存"系统（BookmarkCache/静默收集）→ 关卡数据化后时刻在数据里，书签直接读；保留人工打点
- [ ] BookmarkExtractor（静态提取）→ 收集系统被取代后仅剩兜底
- [ ] stage01.gd 编排层代码（tl.at 链）→ 迁移后编排代码冗余（敌人协程脚本保留，成为注册表脚本层）
- [ ] CONTENT_GUIDE.md 制作流程 → 重写为填表/注册
- 过渡期双轨（数据关卡 + 协程关卡并存）属正常；协程脚本/人工打点/基础设施/测试均不冗余
