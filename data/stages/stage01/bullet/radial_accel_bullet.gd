...
func _tick(ctx: StageContext) -> Variant:
...
func _spawn_downward(bullet: Bullet, ctx: StageContext) -> void:
	...
	data.tint_mode = bullet.tint_mode
	data.damage = bullet.damage
	data.hitbox_shape = bullet.hitbox_shape
	...
	ctx.bullets.shoot_single(data, Vector2(bullet.global_position.x, GameConfig.FIELD_TOP), Vector2.DOWN)
