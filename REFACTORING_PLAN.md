...
- [ ] **内容工作台（Content Workbench）** —— 见下方设计蓝图

**验收**：5000 弹同屏 60fps；新符卡从设计到可玩 < 1 小时。

---

## 🛠️ 内容工作台设计蓝图（2026-07-31 · 灵感来自 YiHe）

> 目标：**以对象为中心、以对象生命周期为时间轴**的内容预览/编辑/导出工具。
> 技术地基已全部就绪：CoroutineRunner 可暂停时钟、RNG 可复现、数据驱动 .tres、HitboxVisualizer、GameConfig 边界。
...
- 单一种弹幕（BulletData）
- 一个敌人（EnemyData）
- 一张符卡/非符（PhaseData / BossData）
- 一个完整关卡（StageData，含背景）

**播放 = 真实实例化**：不是模拟动画，走真实生成逻辑（BulletPool/协程/掉落）。
**嵌套自动成立**：特殊敌人→生成特殊弹幕，子对象自然运行（免费获得，因为走的是真实逻辑）。
**时间以对象为锚点**：围绕选中对象播放/暂停/单步/重置，而非跑完整关卡。
...
┌─────────────────────────────────────────────┐
│  ① 对象选择栏（资源面板）                     │
│     BulletData / EnemyData / PhaseData /     │
│     BossData / StageData                     │
├─────────────────────────────────────────────┤
│  ② 沙盒舞台（干净场景）                       │
│     · 东方框背景（GameConfig）                │
│     · 可选网格 + 命中框显示（HitboxVisualizer）│
│     · 子弹计数 / 帧率 / 经过时间               │
├─────────────────────────────────────────────┤
│  ③ 时间锚点控制条                             │
│     ▶播放 ⏸暂停 ⏭单步 ↺重置 🐢速度×0.5      │
│     · 绑定沙盒 CoroutineRunner 时钟            │
│     · RNG 种子显示 + 一键重掷                  │
├─────────────────────────────────────────────┤
│  ④ 生命周期进度（对象的时间轴）               │
│     弹幕：发射→出屏   敌人：生成→死亡          │
