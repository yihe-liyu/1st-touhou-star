...
- **工作台 = 真实实例化**（走真实 BulletPool/协程/掉落逻辑），所见即所得
- **确定性来自 RNG 种子**：重跑 = 重置种子 + 重新实例化 → 结果一致
- **时间轴拖动到 t** = 从 0 确定性重跑到 t → 渲染那一刻全树状态
- 模型与运行时实体不分离——**真实逻辑本身就是确定性模拟**（项目 RNG 保证）
...
| 导出 | 说明 | 实现 |
|------|------|------|
| 💾 **资源蓝图** | 调好的对象存成 .tres | `ResourceSaver.save()` |
