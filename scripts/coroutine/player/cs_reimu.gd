...
const MAIN_BULLET = preload("res://data/bullet_data/reimu_main_bullet.tres")
const OPTION_BULLET = preload("res://data/bullet_data/reimu_option_bullet02.tres")
...
func _on_run(api: StageAPI):
	...
	while api._active():
		...
		if shooting:
			...
			api.shoot_spread(MAIN_BULLET, 1, 0.0, Vector2.UP, player.global_position + Vector2(20, 0))
			if pw < 100:
				_shoot_options(api, OPTION_BULLET, 1, 0.0, Vector2.UP)
				await api.frames(5)
			elif pw < 200:
				_shoot_options(api, OPTION_BULLET, 1, 0.0, Vector2.UP)
				await api.frames(5)
			else:
				_shoot_options(api, OPTION_BULLET, 3, deg_to_rad(10), Vector2.UP)
				await api.frames(4)
		else:
			await api.frames(1)
