...
func spawn_enemy(data: EnemyData, pos: Vector2) -> Timeline:
	return do(func(): ctx.enemies.spawn_enemy(data, pos))

func spawn_boss(data: BossData, pos: Vector2) -> Timeline:
	return do(func(): ctx.enemies.spawn_boss(data, pos))

func play_bgm(path: String) -> Timeline:
	return do(func(): AudioManager.play_bgm(load(path), 0.0))
